...
client.on("guildMemberAdd", member => {
  member.roles.add('768191452214984754'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "768189879317561385");
    const register = "<@&768198610318917642>"
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '△'
  const sunucu = '768179715286237194'
  const kanal = '772151873170964520'
  const rol = '768196196563943425'
